...
export interface ZoomGridProps<T> extends Omit<LegendListProps<T>, 'renderItem'> {
    ...
    renderHeader?: (props: { isSelectionMode?: boolean }) => React.ReactNode;
...
export function ZoomGrid<T>({
    ...
    const [layerConfig, setLayerConfig] = useState<{
        [key: number]: { padding: number; scrollOffset: number; targetIndex: number };
    }>(() => {
        const initialConfig: any = {};
        zoomLevels.forEach(cols => {
            initialConfig[cols] = { padding: 0, scrollOffset: 0, targetIndex: 0 };
        });
    ...
    const listRefs = useRef<{ [key: number]: any }>({});
    ...
    const calculateLayerConfig = (
        ...
        return { padding, scrollOffset: clampedTargetScroll, targetIndex: paddedIndex };
    };

    const handleZoomFinish = (nextCols: number) => {
    ...
    const prepareZoom = (fX: number, fY: number) => {
    ...
    const renderLayer = (cols: number) => {
        const isActive = cols === activeColumns;
        const config = layerConfig[cols] || { padding: 0 };
        ...
        const animatedStyle = useAnimatedStyle(() => {
            ...
            const activeConfig = layerConfig[currentActiveCols] || { padding: 0, scrollOffset: 0, targetIndex: 0 };
        ...
        return (
            <Animated.View key={cols} style={[styles.layer, animatedStyle, DEBUG ? { borderColor: color, borderWidth: 2 } : {}]} pointerEvents={isActive ? 'auto' : 'none'}>
                <ZoomGridList
                    ref={(r: any) => listRefs.current[cols] = r}
                    data={layerData}
                    numColumns={cols}
    ...
    return (
        <GestureHandlerRootView style={[styles.container, gridStyle]}>
            {renderHeader && renderHeader({})}
            <GestureDetector gesture={pinch}>
                <View style={[styles.container, gridStyle]}>
                    {zoomLevels.map(renderLayer)}
                </View>
            </GestureDetector>
